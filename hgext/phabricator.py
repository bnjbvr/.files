...
sys.path.append(
    '%(HOME)s/.mozbuild/version-control-tools/pylib/mozautomation/'
    % os.environ)
...
def user_group_phids(repo, names):
    ...
    results = [entry[r'phid'] for entry in data]
    unresolved = set(names) - resolved
    if unresolved:
        # Check group slugs
        query = {b'constraints': {b'slugs': list(unresolved)}}
        result = callconduit(repo, b'project.search', query)
        for slug, m in result[r'maps'].get(r'slugMap', {}).iteritems():
            if slug in unresolved:
                unresolved.remove(slug)
                results.append(m[r'projectPHID'])
    if unresolved:
...
def getdiffmeta(diff):
    ...
    if not meta and props.get(r'local:commits'):
        ...
        meta = {
            r'date': r'%s 0' % commit[r'time'],
            r'node': commit.get(r'rev') or commit.get(r'commit'),
